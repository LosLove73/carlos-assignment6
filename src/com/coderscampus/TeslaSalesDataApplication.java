...
public class TeslaSalesDataApplication {
	...
	public List<TeslaSalesData> filterSalesDataByModel(List<TeslaSalesData> salesData, String model) {
		return salesData.stream()
				.filter(data -> data.getModel().equalsIgnoreCase(model.trim()))
				.collect(Collectors.toList());
	}
	
	 public void printYearlySalesReport(String model, List<TeslaSalesData> modelSales) {
	        Map<Integer, Integer> salesByYear = modelSales.stream()
	                .collect(Collectors.groupingBy(
	                        data -> Year.from(data.getDate()).getValue(),
	...
	public void printTheBestAndWorstMonthsOfSales(List<TeslaSalesData> modelSales, String model) {
        ...
        Optional<TeslaSalesData> bestMonth = modelSales.stream()
                .max(Comparator.comparing(TeslaSalesData::getSales));

        Optional<TeslaSalesData> worstMonth = modelSales.stream()
                .min(Comparator.comparing(TeslaSalesData::getSales));

        bestMonth.ifPresentOrElse(data ->
                System.out.println("The best month for " + model + " was: " +
                        YearMonth.from(data.getDate()).format(formatter) + " with " +
                        data.getSales() + " units sold."),
                () -> System.out.println("No sales data available for best month of " + model)
        );

        worstMonth.ifPresentOrElse(data ->
